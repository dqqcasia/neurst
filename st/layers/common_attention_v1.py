...
from st.layers import common_layers_v1
import sys
...
#     """
#     attn = tf.Print(attn, [attn, tf.shape(attn)], "[attn, tf.shape(attn)]", summarize=20)
...
def dot_product_attention(q,
    ...
    with tf.variable_scope(
        ...
        logits = tf.matmul(q, k, transpose_b=True)  # [batch_size, 8, length_q, length_kv]
        # bias:
        if bias is not None:
            logits += bias
        weights = tf.nn.softmax(logits, name="attention_weights")
        # print attention weights
        # weights = tf.Print(weights, [tf.shape(weights), weights], "tf.shape(weights), weights", summarize=10000000)
...
def multihead_attention(query_antecedent,
    ...
    with tf.variable_scope(
        ...
        if memory_antecedent is None:
            # self attention
            combined = common_layers_v1.conv1d(
                query_antecedent,
        ...
        x = combine_heads(x)
        x = common_layers_v1.conv1d(x, output_depth, 1, name="output_transform")
        return x
