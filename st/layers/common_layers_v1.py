...
from st.layers import expert_utils_v1 as eu
...
def standardize_images(x):
    ...
    with tf.name_scope("standardize_images", [x]):
        ...
        # TODO(lukaszkaiser): remove hack below, needed for greedy decoding for
        # now.
        if x.shape and len(x.shape) == 4 and x.shape[3] == 1:
            # Not used, just a dead tf.cond branch.
...
def embedding(x, vocab_size, dense_size, name=None, reuse=None, multiplier=1.0):
    ...
    with tf.variable_scope(
        ...
        return tf.reshape(emb_x, [shape[0], shape[1],
                                  shape[2], static_shape[4]])
...
def conv_internal(conv_fn, inputs, filters, kernel_size, **kwargs):
    ...
    inputs.set_shape = tf.TensorShape(
        [static_shape[0], None, None, static_shape[3]])  # pylint: disable=protected-access
...
def conv(inputs, filters, kernel_size, **kwargs):
    return conv_internal(tf.layers.conv2d, inputs,
        ...
        if separability:
            ...
            if separability > 1:
                result = tf.layers.conv2d(
                    tf.concat(parts, axis=3), filters, (1, 1))
...
def conv_block_internal(conv_fn,
    ...
    if norm is None and "normalizer_fn" not in kwargs:
        def norm(x, name): return layer_norm(x, filters, name=name)
...
def decompress_seqcnn(x,
    ...
    with tf.variable_scope(
        ...
        # Flatten x and embedded targets. Flat targets are factor* larger on
        # axis=1.
        ...
        # Run a SeqCNN large-batch to produce factor outputs out of every
        # target.
...
def pad_to_same_length(x, y, final_length_divisible_by=1, axis=1):
    ...
    with tf.name_scope("pad_to_same_length", [x, y]):
        ...
        if final_length_divisible_by > 1:
            # Find the nearest larger-or-equal integer divisible by given
            # number.
...
def padded_cross_entropy(logits,
    ...
    with tf.name_scope("padded_cross_entropy", [logits, labels]):
        ...
        xent = smoothing_cross_entropy(
            logits, pad_labels, vocab_size, confidence)
