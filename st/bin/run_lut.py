...
from st.tools.performanceTools import dev_test
...
def train():
    ...
    logging.info('reading data form %s'.format(args.dirs.train.tfdata))
    ...
    def prepare_train(args):
        ...
        if args.model.use_multilabel:
            if args.model.semantic_encoder_pretrain:
                batch_train = dataReader_train.fentch_multi_label_batch_bucket_nmt()
            else:
                batch_train = dataReader_train.fentch_multi_label_batch_bucket()

            _, feat_dev, label_dev, auxlabel_dev = readTFRecord_multilabel(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            dataloader_dev = ASRMultiLabelDataLoader(args.dataset_dev, args, feat_dev, label_dev, auxlabel_dev, batch_size=args.batch_size,
                                                     num_loops=1)

            _, feat_test, label_test, auxlabel_test = readTFRecord_multilabel(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_dev, label_dev, bertfeat_dev = readTFRecord_kdbert(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_test, label_test, bertfeat_test = readTFRecord_kdbert(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_dev, label_dev, auxlabel_dev, bertfeat_dev = readTFRecord_ctcbert(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_test, label_test, auxlabel_test, bertfeat_test = readTFRecord_ctcbert(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_dev, label_dev, auxlabel_dev, bertfullfeat_dev = readTFRecord_ctcbertfull(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_test, label_test, auxlabel_test, bertfullfeat_test = readTFRecord_ctcbertfull(args.dirs.test.tfdata, args, _shuffle=False,
                                                                                               transform=True)
            ...
            _, feat_dev, label_dev, auxlabel_dev = readTFRecord_multilingual(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_test, label_test, auxlabel_test = readTFRecord_multilingual(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_dev, label_dev, label_dev_2, auxlabel_dev = readTFRecord_multilingual_multilabel(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_test, label_test, label_test_2, auxlabel_test = readTFRecord_multilingual_multilabel(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
            ...
            _, feat_dev, label_dev = readTFRecord(args.dirs.dev.tfdata, args, _shuffle=False, transform=True)
            dataloader_dev = ASRDataLoader(args.dataset_dev, args, feat_dev, label_dev, batch_size=args.batch_size, num_loops=1)

            _, feat_test, label_test = readTFRecord(args.dirs.test.tfdata, args, _shuffle=False, transform=True)
    ...
    if args.dirs.kd_teacher and args.dirs.kd_teacher.checkpoint:
        pretrain_vars = list_variables(args.dirs.kd_teacher.checkpoint)
        if args.dirs.kd_teacher.scope:
            pretrain_layers = args.dirs.kd_teacher.scope.split(',')
            pretrain_vars_teacher_list = []
            for var_pre in pretrain_vars:
                for layer in pretrain_layers:
                    if var_pre[0].startswith(layer):
                        pretrain_vars_teacher_list.append(var_pre[0])
        else:
            pretrain_vars_teacher_list = [var_pre[0] for var_pre in pretrain_vars]

        vars_list = tf.all_variables()
        restored_list = []
        for var in vars_list:
            if var.name.split(':')[0] in pretrain_vars_teacher_list and var.name.split(':')[0] != 'global_step':
                restored_list.append(var)

        saver_teacher = tf.train.Saver(restored_list)
    ...
    with tf.train.MonitoredTrainingSession(config=config) as sess:
        ...
        if args.dirs.kd_teacher and args.dirs.kd_teacher.checkpoint:
            saver_teacher.restore(sess, args.dirs.kd_teacher.checkpoint)
        ...
        def evaluate_decode(args, model_infer, sess, global_step, dataloader_dev, dataloader_test):
        ...
        global_step = 0
        dev_cer, dev_wer, dev_bleu = evaluate_decode(args, model_infer, sess, global_step, dataloader_dev, dataloader_test)
        ...
        while progress < args.num_epochs:
            ...
            if args.model.use_multilabel and not args.model.semantic_encoder_kdmt:
                [loss, shape_batch, _, _], loss_ast, loss_asr = sess.run([model.list_run,
                                                                     model.loss_ast,
                                                                     model.loss_am])
                loss_bert = 0
                loss_ctc = loss_asr
                summary.summary_scalar('loss_ctc', loss_asr, global_step)
                summary.summary_scalar('loss_ast', loss_ast, global_step)
                # tensor_test, loss_ctc, loss = sess.run([batch_train, model.summary_loss_ctc, model.loss])
            elif args.model.use_bert:
                [loss, shape_batch, _, _], loss_ast, loss_bert = sess.run([model.list_run,
                                                                        model.loss_ast,
                                                                        model.loss_bert])

                loss_ast, loss_ctc, loss_bert = 0, 0, 0
            elif args.model.use_ctc_bert:
                ...
                summary.summary_scalar('loss_ctc', loss_ctc, global_step)
                loss_bert = 0
            elif args.model.semantic_encoder_kdmt:
                [loss, shape_batch, _, _], loss_ast, loss_ctc, loss_bert = sess.run([model.list_run,
                                                                                     model.loss_ast,
                                                                                     model.loss_am,
                                                                                     model.loss_bert])
            ...
            if args.begin_dev_step and global_step < args.begin_dev_step:
                ...
                elif global_step % 1000 ==0 and global_step != 0:
                    model_path = args.dir_checkpoint + '/model'
                    saver.save(
                        get_session(sess),
                        model_path,
...
def infer():
    ...
    with tf.train.MonitoredTrainingSession(config=config) as sess:
        ...
        def run_decode(dataset, model, sess):
            batch_time = time()
            start_time = time()
            with tf.io.gfile.GFile(args.dir_model+'/test_wav.txt', 'w') as fwav, \
                ...
                while progress < 1:
                    if args.model.loss_type == 'ctc_kd_ce' or args.model.loss_type == "CTC_BERT" or args.model.loss_type == "ce_ctc" or args.model.loss_type == 'ce_ctc_kd':
            ...
            if args.metrics == 'BLEU':
                ...
                bleu = 0
                wer = calc_wer('test_ast_ref.txt', 'test_ast_res.txt')
            ...
            if args.model.loss_type == 'ctc_kd_ce' or args.model.loss_type == "CTC_BERT" or args.model.loss_type == "ce_ctc":
                ...
                ctc_wer = calc_wer('test_asr_ref.txt', 'test_asr_res.txt')
                return ctc_wer, wer, bleu
            else:
                ctc_wer = 0
                return ctc_wer, wer, bleu
        
        if args.model.loss_type == 'ctc_kd_ce' or args.model.loss_type == "CTC_BERT" or args.model.loss_type == "ce_ctc":
            ctc_wer, test_wer, test_bleu = run_decode(args, model_infer, sess)
            ...
            logging.info('Test Infer CTC_WER: {:.3f} \t WER: {:.3f} \t BLEU: {:.3f}'.format(ctc_wer, test_wer, test_bleu))
        else:
            _, test_wer, test_bleu = run_decode(args, model_infer, sess)
            logging.info('infer checkpoint name: {}'.format(checkpoint_name))
            logging.info('beam_size: {} \t length_penalty_weight: {} \t max_len: {}'.format(args.beam_size, args.length_penalty_weight, args.max_len))
            logging.info('Test Infer WER: {:.3f} \t BLEU: {:.3f}'.format(test_wer, test_bleu))
    ...
    with tf.train.MonitoredTrainingSession(config=config) as sess:
        ...
        with tf.io.gfile.GFile(args.dir_model + '/test_wav.txt', 'w') as fwav, \
            ...
            for sample in tqdm(args.dataset_test):
                ...
                fastres.write(''.join(res_txt) + '\n')
                fastref.write(''.join(ref_txt) + '\n')

                if args.model.loss_type == 'ctc_kd_ce' or args.model.loss_type == "CTC_BERT" or args.model.loss_type == "ce_ctc":
                    token_line = array2text(decoded_am[0], args.data.unit, args.src_idx2token, min_idx=0, max_idx=args.dim_output - 1, args=args, delimiter=args.data.src_delimiter)
                    fasrres.write(''.join(token_line).replace("<eos>", '') + '\n')
                    token_line = array2text(sample['aux_label'], args.data.unit, args.src_idx2token, min_idx=0, max_idx=args.dim_output - 1, args=args, delimiter=args.data.src_delimiter)
                    fasrref.write(''.join(token_line).replace("<eos>", '') + '\n')
        ...
        if args.model.loss_type == 'ctc_kd_ce' or args.model.loss_type == "CTC_BERT" or args.model.loss_type == "ce_ctc":
            ...
            wer = calc_per('test_asr_ref.txt', 'test_asr_res.txt')
            logging.info('infer checkpoint name: {}'.format(checkpoint_name))
            logging.info(
                'beam_size: {} \t length_penalty_weight: {} \t max_len: {}'.format(args.beam_size, args.length_penalty_weight, args.max_len))
            logging.info('infer CTC_WER: {:.3f} \t BLEU: {:.3f}'.format(
                wer,
                bleu))
        else:
            logging.info('infer checkpoint name: {}'.format(checkpoint_name))
            logging.info('beam_size: {} \t length_penalty_weight: {} \t max_len: {}'.format(args.beam_size, args.length_penalty_weight, args.max_len))
            logging.info('infer BLEU: {:.3f}'.format(
                bleu))
    ...
    with tf.train.MonitoredTrainingSession(config=config) as sess:
        ...
        def run_decode(dataset):
            ...
            for sample in tqdm(dataset):
                print(sample['id'])
                if not sample or sample['id'] != args.wav_name:
                    continue
                dict_feed = {model.list_pl[0]: np.expand_dims(sample['feature'], axis=0),
                             model.list_pl[1]: np.array([len(sample['feature'])])}

                train_summary = sess.run(model.summary_op, feed_dict=dict_feed)
                summary.writer.add_summary(train_summary, idx)
                idx += 1
